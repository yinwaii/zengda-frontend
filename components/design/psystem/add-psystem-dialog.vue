<template>
	...
	<!-- PSYSTEM 创建对话框 -->
	<psystem-dialog :open="showCreateDialog" :psystem="psystem" @submit="handlePsystemCreated" />
</template>
...
const handleSubmit = async () => {
	if (selectedPsystemId.value) {
		const parentPsystem = await entityApis.psystem.get(props.parentId)
		const psystem = await entityApis.psystem.get(selectedPsystemId.value)
		console.log(props.parentId, parentPsystem, selectedPsystemId.value, psystem)
		parentPsystem.children.push(psystem)
		await entityApis.psystem.update(parentPsystem)
