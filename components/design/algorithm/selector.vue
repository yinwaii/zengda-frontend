...
const selectModule = async () => {
	if (selected.value) {
		const rawData: any = await $fetch('/api/modules/getComputedForModule', { query: { id: selected.value.at(-1) } });
		rawData.form = await $fetch('/api/modules/getModuleArgumentList', { query: { id: selected.value.at(-1) } });
		rootArgument.value = rawData;
	}
}
</script>
