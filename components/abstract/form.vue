	...
	<el-form label-position="left" label-width="80px">
		<template v-for="(value, key) in $props.param">
			...
			<el-form-item v-else :label="value?.name" label-position='left'>
				...
				<el-input size="small" v-model="getParamItem(model, value)[key]" v-else-if="value?.type === 'string'" />
				<el-switch size="small" v-model="getParamItem(model, value)[key]" v-else-if="value?.type === 'boolean'" />
				<el-button size="small" type="primary" v-else-if="value?.type === 'file'">上传</el-button>
				<el-select size="small" v-model="getParamItem(model, value)[key]" v-else-if="value?.type?.startsWith('selector')">
					<el-option v-for="item in splitSelector(value?.type)" :key="item" :label="item" :value="item" />
				</el-select>
				<!-- <el-select size="small" v-model="getParamItem(model, value)[key]" v-else-if="value?.type === 'type'">
